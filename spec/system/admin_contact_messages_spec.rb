...
RSpec.describe 'Admin Contact Messages Interface', type: :system do
  ...
  describe 'email template functionality', js: true do
    ...
    it 'shows Rails email form when Send via Rails is clicked' do
    ...
    it 'can cancel Rails email form' do
      fill_in 'reply-textarea', with: 'Test reply message'
      click_button 'Send via Rails'
      click_button 'Cancel'
  ...
  describe 'status management' do
    it 'marks message as read when clicked' do
      visit admin_contact_message_path(pending_message)
      click_link 'Mark as Read'

      expect(page).to have_content('Message marked as read')
      expect(pending_message.reload.status).to eq('read')
    end
  ...
  describe 'bulk actions' do
    ...
    it 'shows error when no messages selected' do
      select 'Mark as Read', from: 'bulk_action'
      click_button 'Apply'

      expect(page).to have_content('No messages selected')
    end
  end
  ...
  describe 'navigation between messages' do
    ...
    it 'shows navigation between messages' do
      visit admin_contact_message_path(message2)
